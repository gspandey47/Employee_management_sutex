...
const Rejected_leaves = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost/my-backend/desplayrejectedleaves.php")
      .then(response => {
...
//   };
  const handleDelete = (employee_id) => {
    if (window.confirm("Are you sure you want to delete this shift request?")) {
      axios.post(
        "http://localhost/my-backend/deleterejectleave.php",
        { employee_id },
        { headers: { "Content-Type": "application/json" }, withCredentials: false }
      )
      .then(response => {
  ...
  return (
    <div className="container mx-auto p-6 bg-gray-100">
      <h2 className="text-3xl font-bold text-center text-blue-600 mb-4">Rejected Leaves Requests</h2>
